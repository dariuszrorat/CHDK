...
// length of printed string is always 4
    ...
    else
    {
        ...
        if (i == 0)
            ...
            sprintf(buf, "%d.%02dm", i, (f+5)/10);
        else if (i < 100)
            sprintf(buf, "%02d.%dm", i, (f+50)/100);
        else
            sprintf(buf, "%4dm", i);
    }
}

static void print_aov(char *buf, int angle)
{
    int i = angle / 10;
    int f = angle % 10;
...
void gui_osd_draw_dof(int is_osd_edit)
{
              ...
              ((camera_info.state.mode_video || is_video_recording()) && conf.show_values_in_video))))
    {
        ...
        int focus_mode = shooting_get_real_focus_mode();
        int zp=shooting_get_zoom(), fl=get_focal_length(zp);
        double focal_length = (double) fl / 1000.0;
        double aov_rad_h, aov_rad_v, aov_rad_d;

        if ((camera_info.dof_values.subject_distance <= 0) || shooting_is_infinity_distance())
        {
        ...
        {
            double S1 = (double) camera_info.dof_values.subject_distance;
            double S2 = S1 * focal_length / (S1 - focal_length);
            aov_rad_h = 2.0 * arctan(CAM_SENSOR_WIDTH / (2.0 * S2), 19);
            aov_rad_v = 2.0 * arctan(CAM_SENSOR_HEIGHT / (2.0 * S2), 19);
            aov_rad_d = 2.0 * arctan(CAM_SENSOR_DIAGONAL / (2.0 * S2), 19);
        }

        int aov_deg_h = (int) Round(10 * aov_rad_h * 180.0 / M_PI, 0);
        int aov_deg_v = (int) Round(10 * aov_rad_v * 180.0 / M_PI, 0);
        int aov_deg_d = (int) Round(10 * aov_rad_d * 180.0 / M_PI, 0);

        int fov_h = (int) Round(2.0 * (double) camera_info.dof_values.subject_distance * tang(aov_rad_h / 2.0), 0);
        int fov_v = (int) Round(2.0 * (double) camera_info.dof_values.subject_distance * tang(aov_rad_v / 2.0), 0);
        int fov_d = (int) Round(2.0 * (double) camera_info.dof_values.subject_distance * tang(aov_rad_d / 2.0), 0);

        //print AOV and FOV values
        draw_osd_string(conf.dof_pos, 0, 2*FONT_HEIGHT, "AOV H/V/D:", col, conf.dof_scale);
        i = 10;
        sprintf_aov(osd_buf, aov_deg_h);
        j = strlen(osd_buf);
        draw_osd_string(conf.dof_pos, i*FONT_WIDTH, 2*FONT_HEIGHT, osd_buf, col, conf.dof_scale);
        i = i+j;
        draw_osd_string(conf.dof_pos, i*FONT_WIDTH, 2*FONT_HEIGHT, "/", col, conf.dof_scale);
        sprintf_aov(osd_buf, aov_deg_v);
        draw_osd_string(conf.dof_pos, (++i)*FONT_WIDTH, 2*FONT_HEIGHT, osd_buf, col, conf.dof_scale);
        j = strlen(osd_buf);
        i = i + j;
        draw_osd_string(conf.dof_pos, i*FONT_WIDTH, 2*FONT_HEIGHT, "/", col, conf.dof_scale);
        sprintf_aov(osd_buf, aov_deg_d);
        draw_osd_string(conf.dof_pos, (++i)*FONT_WIDTH, 2*FONT_HEIGHT, osd_buf, col, conf.dof_scale);

        draw_osd_string(conf.dof_pos, 0, 3*FONT_HEIGHT, "FOV H/V/D:", col, conf.dof_scale);
        i = 10;
        sprintf_dist(osd_buf, fov_h);
        j = strlen(osd_buf);
        draw_osd_string(conf.dof_pos, i*FONT_WIDTH, 3*FONT_HEIGHT, osd_buf, col, conf.dof_scale);
        i = i+j;
        draw_osd_string(conf.dof_pos, i*FONT_WIDTH, 3*FONT_HEIGHT, "/", col, conf.dof_scale);
        sprintf_dist(osd_buf, fov_v);
        draw_osd_string(conf.dof_pos, (++i)*FONT_WIDTH, 3*FONT_HEIGHT, osd_buf, col, conf.dof_scale);
        j = strlen(osd_buf);
        i = i + j;
        draw_osd_string(conf.dof_pos, i*FONT_WIDTH, 3*FONT_HEIGHT, "/", col, conf.dof_scale);
        sprintf_dist(osd_buf, fov_d);
        draw_osd_string(conf.dof_pos, (++i)*FONT_WIDTH, 3*FONT_HEIGHT, osd_buf, col, conf.dof_scale);
    ...
    if (is_osd_edit || (camera_info.state.mode_rec_or_review && conf.show_values))
    {
        ...
        if ((showtype == 1) || is_osd_edit)
        {
            if (conf.values_show_zoom || is_osd_edit)
            {
                ...
                {
                case ZOOM_SHOW_FL:
                    sprintf(osd_buf, "F:%2d.%dmm%8s", fl1/1000, fl1%1000/100, "");
                    gui_print_osd_misc_string();
                    break;
                case ZOOM_SHOW_EFL:
                    ...
                    sprintf(osd_buf, "E: %3dmm%8s", fl/1000, "");
                    gui_print_osd_misc_string();
                    break;
                case ZOOM_SHOW_ALL:
                    // zoom X
                    fl=get_zoom_x(zp);
                    sprintf(osd_buf, "Z:%2d.%dx%8s", fl/10, fl%10, "");
                    gui_print_osd_misc_string();
                    // zoom FL
                    sprintf(osd_buf, "F:%2d.%dmm%8s", fl1/1000, fl1%1000/100, "");
                    gui_print_osd_misc_string();
            ...
            if ((conf3.values_show_color_temp_tint != 0) && camera_info.state.is_shutter_half_press)
            {
                ...
                double temperature;
                double tint;
                RGB[0] = 1.0 / (wb[1] / 1000.0);
                RGB[1] = 1.0 / (wb[0] / 1000.0);
                ...
                if ((conf3.values_show_color_temp_tint % 2) != 0)
                {
                    sprintf(osd_buf, "T:%5dK%8s", tk, "");
                    osd_buf[9] = 0;
                    draw_osd_string(conf.values_pos, 0, m, osd_buf, user_color(conf.osd_color), conf.values_scale);
                    m+=FONT_HEIGHT;
                }

                if (conf3.values_show_color_temp_tint > 1)
                {
                    sprintf(osd_buf, "G:%2d.%03d%8s", tnt / 1000, tnt % 1000, "");
                    osd_buf[9] = 0;
                    draw_osd_string(conf.values_pos, 0, m, osd_buf, user_color(conf.osd_color), conf.values_scale);
                    m+=FONT_HEIGHT;
                }
            ...
            if (camera_info.state.is_shutter_half_press
                    && conf3.values_show_overexp_percent
                    && conf.autoiso_enable
                    && (shooting_get_iso_mode()<=0)
                    && (camera_info.state.mode_shooting==MODE_AUTO || camera_info.state.mode_shooting==MODE_P || camera_info.state.mode_shooting==MODE_AV)
                    && shooting_get_flash_mode()
                    && (autoiso_and_bracketing_overrides_are_enabled)
               )
            {
                sprintf(osd_buf, "OP:%3d%% %8s", vars.overexp_percent, "");
                osd_buf[9] = 0;
                draw_osd_string(conf.values_pos, 0, m, osd_buf, user_color(conf.osd_color), conf.values_scale);
                m+=FONT_HEIGHT;
            }
            ...
            if (camera_info.state.is_shutter_half_press
                    && conf3.values_show_auto_ev_comp
                    && conf.autoiso_enable
                    && (conf.overexp_ev_enum != 0)
            ...
            {
                const char* ae_ev_values[] = { " 0.00 EV", "-0.33 EV", "-0.67 EV", "-1.00 EV", "-1.33 EV", "-1.67 EV", "-2.00 EV" };
                sprintf(osd_buf, "%s %8s", ae_ev_values[vars.auto_ev_ae_value], "");
                osd_buf[9] = 0;
                draw_osd_string(conf.values_pos, 0, m, osd_buf, user_color(conf.osd_color), conf.values_scale);
                m+=FONT_HEIGHT;
                vars.auto_ev_ae_value = 0;
