...
#define CONF2_FILE          "A/CHDK/CCHDK4_2.CFG"
...
void conf2_save()
{
    ...
    FILE *pFile = fopen(CONF2_FILE, "w");
    if (pFile)
    {
        sprintf(buf, "parade_mode=%d\n", conf3.parade_mode);
        fwrite(buf, sizeof(char), strlen(buf), pFile);
        sprintf(buf, "show_color_temp_tint=%d\n", conf3.values_show_color_temp_tint);
        fwrite(buf, sizeof(char), strlen(buf), pFile);
        sprintf(buf, "show_overexp_percent=%d\n", conf3.values_show_overexp_percent);
        fwrite(buf, sizeof(char), strlen(buf), pFile);
        sprintf(buf, "show_auto_ev_comp=%d\n", conf3.values_show_auto_ev_comp);
        fwrite(buf, sizeof(char), strlen(buf), pFile);
        sprintf(buf, "zebra_rgb_fn=%d\n", conf3.zebra_rgb_fn);
        fwrite(buf, sizeof(char), strlen(buf), pFile);
        sprintf(buf, "zebra_zone_file=%s\n", conf2.zebra_zone_file);
        fwrite(buf, sizeof(char), strlen(buf), pFile);
        sprintf(buf, "zebra_zone_title=%s\n", conf2.zebra_zone_title);
        fwrite(buf, sizeof(char), strlen(buf), pFile);
        fclose(pFile);
    }
}

void process_int(const char *str, int n, int *value)
{
    const char *ptr = str;
    char *nptr;
    int i;
...
void process_string(const char *str, int n, char *dest)
{
    const char *ptr = str;
    int i;
...
void parse_line(char *ptr, int maxlen)
{
    int value;
    while (ptr[0])
    {
        while (ptr[0]==' ' || ptr[0]=='\t') ++ptr; // whitespaces
        if (strncmp("parade_mode=", ptr, 11)==0)
        {
            ptr+=11;
            process_int(ptr, maxlen-11-1, &value);
            conf3.parade_mode = value;
            return;
        }
        if (strncmp("show_color_temp_tint=", ptr, 20)==0)
        {
            ptr+=20;
            process_int(ptr, maxlen-20-1, &value);
            conf3.values_show_color_temp_tint = value;
            return;
        }
        if (strncmp("show_overexp_percent=", ptr, 20)==0)
        {
            ptr+=20;
            process_int(ptr, maxlen-20-1, &value);
            conf3.values_show_overexp_percent = value;
            return;
        }
        if (strncmp("show_auto_ev_comp=", ptr, 17)==0)
        {
            ptr+=17;
            process_int(ptr, maxlen-17-1, &value);
            conf3.values_show_auto_ev_comp = value;
            return;
        }
        if (strncmp("zebra_rgb_fn=", ptr, 12)==0)
        {
            ptr+=12;
            process_int(ptr, maxlen-12-1, &value);
            conf3.zebra_rgb_fn = value;
            return;
        }
        if (strncmp("zebra_zone_file=", ptr, 15)==0)
        {
            ptr+=15;
            process_string(ptr, maxlen-15-1, conf2.zebra_zone_file);
            return;
        }
        if (strncmp("zebra_zone_title=", ptr, 16)==0)
        {
            ptr+=16;
            process_string(ptr, maxlen-16-1, conf2.zebra_zone_title);
            return;
        }
    }
}

void conf2_load_defaults()
{
    conf2.zebra_zone_file[0] = 0;
    conf2.zebra_zone_title[0] = 0;
    conf3.parade_mode = 1;
    conf3.values_show_color_temp_tint = 0;
    conf3.values_show_overexp_percent = 0;
    conf3.values_show_auto_ev_comp = 0;
    conf3.zebra_rgb_fn = 0;
...
void conf2_restore()
{
    ...
    FILE *pFile = fopen(CONF2_FILE, "r");
    char buffer[80];
    if (pFile != NULL)
    {
        while (fgets(buffer, 80, pFile) != NULL)
        {
            parse_line(buffer, 80);
        }
